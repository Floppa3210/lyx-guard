--[[
    ...
    Author: LyxDev
    Purpose: Modular punishment handler with rate limiting, auditing and ban hardening.
--]]
...
local function _CreateEvidencePack(source, action, reason, metadata)
    if type(LyxGuardCreateEvidencePack) ~= 'function' then
        return nil
    end

    local ok, path = pcall(LyxGuardCreateEvidencePack, source, {
        action = action,
        reason = reason,
...
PunishmentModule.RegisterHandler(LyxGuardLib.PUNISHMENTS.WARN, function(source, reason, config)
    ...
    if not warningsEnabled then
        ...
        local evidencePath = _CreateEvidencePack(source, 'warning_issued', reason, {
            warning_count = 1,
...
PunishmentModule.RegisterHandler(LyxGuardLib.PUNISHMENTS.KICK, function(source, reason, config)
    ...
    local kickMessage = string.format(
        Config.Punishments and Config.Punishments.messages and Config.Punishments.messages.kick or 'ðŸš« Expulsado: %s',
        reason
    )
...
function BanPlayer(source, reason, duration, bannedBy)
    ...
    local evidencePath = _CreateEvidencePack(source, 'player_banned', reason, {
        duration = duration,
        banned_by = bannedBy,
        permanent = isPermanent,
